...
-- 纪念日（is_lunar=true 时 date 为农历换算后的公历日期）
...
create table if not exists checkins (
  ...
  who         uuid references profiles(id) not null,
...
-- 影视记录（双人评分：rating_a/rating_b + review_a/review_b）
create table if not exists movies (
  id          uuid primary key default gen_random_uuid(),
  couple_id   uuid references couples(id) not null,
  title       text not null,
  poster      text,
  rating_a    numeric(2,1) check (rating_a between 0 and 5),
  rating_b    numeric(2,1) check (rating_b between 0 and 5),
  review_a    text,
  review_b    text,
...
create or replace view timeline as
  select 'diary' as source, id, couple_id, created_at, body as title, author_id as actor_id from diary_entries where not is_deleted
  union all
  select 'anniversary', id, couple_id, date::timestamptz, title, null from anniversaries where not is_deleted
  union all
  select 'movie', id, couple_id, created_at, title, null from movies where not is_deleted
  union all
  select 'plan', id, couple_id, created_at, title, null from plans where not is_deleted
  union all
  select 'checkin', id, couple_id, created_at, type, who from checkins;
...
-- anniversaries
create policy "同空间可读纪念日" on anniversaries for select to authenticated using (couple_id = my_couple() and not is_deleted);
create policy "同空间可写纪念日" on anniversaries for insert to authenticated with check (couple_id = my_couple());
...
create policy "同空间可写计划" on plans for insert to authenticated with check (couple_id = my_couple());
create policy "同空间可改计划" on plans for update to authenticated
  using (couple_id = my_couple()) with check (couple_id = my_couple());

-- tasks
create policy "同空间可读任务" on tasks for select to authenticated using (couple_id = my_couple() and not is_deleted);
create policy "同空间可写任务" on tasks for insert to authenticated with check (couple_id = my_couple());
create policy "同空间可改任务" on tasks for update to authenticated
  using (couple_id = my_couple()) with check (couple_id = my_couple());

-- checkins
create policy "同空间可读打卡" on checkins for select to authenticated using (couple_id = my_couple());
create policy "同空间可写打卡" on checkins for insert to authenticated with check (couple_id = my_couple() and who = auth.uid());
create policy "同空间可改打卡" on checkins for update to authenticated
  using (couple_id = my_couple() and who = auth.uid())
  with check (couple_id = my_couple() and who = auth.uid());

-- diary_entries
create policy "同空间可读日记" on diary_entries for select to authenticated using (couple_id = my_couple() and not is_deleted);
create policy "本人可写日记" on diary_entries for insert to authenticated with check (couple_id = my_couple() and author_id = auth.uid());
create policy "本人可改日记" on diary_entries for update to authenticated
  using (couple_id = my_couple() and author_id = auth.uid())
  with check (couple_id = my_couple() and author_id = auth.uid());
...
create policy "本人可写照片" on diary_photos for insert to authenticated
  with check (entry_id in (select id from diary_entries where couple_id = my_couple() and author_id = auth.uid()));
create policy "本人可删照片" on diary_photos for delete to authenticated
  using (entry_id in (select id from diary_entries where couple_id = my_couple() and author_id = auth.uid()));
...
create policy "同空间可改影视" on movies for update to authenticated
  using (couple_id = my_couple()) with check (couple_id = my_couple());
