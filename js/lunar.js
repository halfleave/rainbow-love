// 农历转换（展示用，覆盖 1900-2100）
// 经典 LUNAR_INFO 算法；仅用于纪念日"农历"标签与倒计时展示，
// 误差仅影响标签文字，不影响核心倒计时逻辑。
const LUNAR_INFO = [
...
export function solarToLunar(date) {
  ...
  for (let i = 1; i < 13 && offset > 0; i++) {
    if (leap > 0 && i === (leap + 1) && !isLeap) {
      temp = leapDays(lunarYear); isLeap = true; i--;
    } else {
      temp = monthDays(lunarYear, i);
    }
    if (!isLeap || i !== leap + 1) offset -= temp;
    else i--;
  }
...
export function formatLunar(date) {
  try {
    ...
    const m = l.isLeap ? '闰' + LUNAR_MONTHS[l.month - 1] : LUNAR_MONTHS[l.month - 1];
    return `农历 ${m}月${LUNAR_DAYS[l.day - 1]}`;
  } catch (e) { return ''; }
}

// 下一个"该公历月日"对应的日期（用于纪念日/周年倒计时），from 之后（含当年或次年）
export function nextOccurrence(month, day, from = new Date()) {
  const y = from.getFullYear();
  let cand = new Date(y, month - 1, day);
  if (cand < from) cand = new Date(y + 1, month - 1, day);
  // 若该日期不存在（如 2/29），顺延到月末
  if (cand.getMonth() !== month - 1) cand = new Date(y + (cand < from ? 1 : 0), month - 1, 0);
  return cand;
}
